...
class ContentBasedRecommender:
    ...
    def _get_similar_items_to_user_profile(self, person_id,source='mindplex',topn=1000):
        ...
        cosine_similarities = cosine_similarity(self.user_profiles[person_id], 
                                    self.model['tfidf'][source])
        #Gets the top similar items
        similar_indices = cosine_similarities.argsort().flatten()[-topn:]
        #Sort the similar items by similarity
        similar_items = sorted([(self.model['ids'][source], cosine_similarities[0,i])\
                                for i in similar_indices],
                key=lambda x: -x[1])
        return similar_items
        
    def recommend_items(self,recommend_from='mindplex', user_id, items_to_ignore=[], topn=10, verbose=False):
